function handler(e) {
  ...
  console.log("___ handler datepicker ____");
  let nueva_fecha = e.target.value;
  cargar_body(nueva_fecha);
}

function atrasos() {
  console.log("ACTUALIZNDO DESPACHOS ATRASADOS");
  $.ajax({
    url: "/despachos_atrasados_defecto",
    type: "POST",
    success: function (res) {
      console.log("ahi respuesta del server");
      console.log(res);
      $("#bol_atraso").text(res.len_boletas[0]);
      $("#fact_atraso").text(res.len_facturas[0]);
      $("#guia_atraso").text(res.len_guias[0]);
    },
  });
}
...
function ver_bol_fact(
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      a =
        '<div class="row"><div class="col col-sm"><table id="table_info1"><thead><td>';
      b =
        "DESCRIPCIÓN</td><td>CANTIDAD</td><td>CANTIDAD RETIRADA</td></thead></table></div></div>";
      items_lista = buscar_adj(tipo_doc, folio);
      //ADJUNTOS
      c =
        '<div class="text-center">Documentos Adjuntos <span class="badge badge-info"> ' +
        items_lista[1].toString() +
        '</span> <button class="btn btn-secondary" onclick="ver_adjuntos()">Ver</button><div class="list-group" id= "lista_adjuntos" style="display:None;">';
      d = "</div>";
      e = "";
      f = "";
      g = "";
      if (tipo_usuario == "porteria") {
        e =
          '<div class="input-group" style="margin-top:10px;"><div class="custom-file"><input type="file" accept="image/*" class="custom-file-input" id="archivo" required><label class="custom-file-label" for="nombre_archivo">';
        f =
          'Elegir o Tomar Foto</label></div><div class="input-group-append"><button class="btn btn-outline-secondary" type="button"';
        g =
          "onclick = adjuntar(" +
          folio +
          "," +
      ...
      if (vinculos) {
        ...
        if (vinculos.guias) {
          ...
          for (let p = 0; p < vinculos.guias.length; p++) {
            dguia =
              dguia +
              '<a href="/documentos/guias/' +
              vinculos.guias[p].toString() +
              '"  class="myButton2" >' +
              vinculos.guias[p].toString() +
              "</a>";
          }
        ...
        if (vinculos.creditos) {
          ...
          for (let p = 0; p < vinculos.creditos.length; p++) {
            dcredito =
              dcredito +
              '<a href="/documentos/creditos/' +
              vinculos.creditos[p].toString() +
              '"  class="myButton2" >' +
              vinculos.creditos[p].toString() +
              "</a>";
          }
      ...
      h =
        '<div class="text-center">Vinculos <span class="badge badge-info">' +
        nro_vinc.toString() +
        '</span> <button class="btn btn-secondary" onclick="ver_vinculos()"> Ver </button><ul class="list-group" id= "lista_vinculos" style="display: None;">';
      k = "</ul></div>";
      items_historial = buscar_historial(tipo_doc, folio);
      y =
        '<div >Historial de retiros <span class="badge badge-info">' +
        items_historial[1].toString() +
        '</span> <button class="btn btn-secondary" onclick="ver_historial()"> Ver </button><ul class="list-group" id= "lista_historial" style="display: None;">';
      z = "</ul></div>";
      $(".modal-body").append(
        a +
          b +
      ...
      for (i = 0; i < resp.length; i++) {
        ...
        ab =
          ...
          '">' +
          maxim +
          '</td><td><button type="button" class="btn btn-light" onclick=reducir(' +
          fila +
          "," +
          ret +
          ") >-</button>";
        cd =
          '<input class= "myinput" type="number" id="item_ret_' +
          fila +
          '" value="' +
          ret +
          '"><button type="button" class="btn btn-light" onclick=aumentar(' +
          fila +
          "," +
          maxim +
          ") >+</button></td></tr>";
      ...
      if (tipo_usuario == "porteria") {
        ...
        m =
          '<button class="btn btn-secondary" onclick=guardar_cambios("' +
          tipo_doc +
          '","' +
          folio +
  ...
  h1 =
    '<div id="left-header"><h4 class="modal-title">' +
    tipo_doc +
    ": " +
    folio +
    "  |  Total: $" +
  ...
  if (tipo_usuario == "porteria") {
    h2 =
      '<button class="btn btn-secondary" style="margin-left:5px;" onclick= dar_baja()>Dar de Baja</button>';
  }
  h3 =
  ...
  $.ajax({
    ...
    success: function (resp) {
      ...
      if (detalle.tipo_doc == "FACTURA") {
        i =
          '<li class="list-group-item">FACTURA: <a href="/documentos/facturas/' +
          detalle.doc_ref.toString() +
          '"  class="myButton2" >' +
          detalle.doc_ref.toString() +
          "</a></li>";
        nro_vinc = 1;
        ...
        i =
          '<li class="list-group-item">BOLETA: <a href="/documentos/boletas/' +
          detalle.doc_ref.toString() +
          '"  class="myButton2" >' +
          detalle.doc_ref.toString() +
          "</a></li>";
      ...
      if (tipo_usuario == "porteria") {
        m =
          '<button class="btn btn-secondary" onclick=guardar_cambios_2(' +
          interno.toString() +
          ") >Guardar Cambios</button>";
      }
      $(".modal-footer").append(l + m);
...
function guardar_cambios(tipo_doc, folio) {
  ...
  if (estado == false) {
    alert(
      "Un item supera la cantidad comprada, tiene un valor negativo o un campo esta incompleto"
    );
    console.log(
      "Un item supera la cantidad comprada o tiene un valor negativo"
    );
...
function buscar_historial(tipo, folio) {
  ...
  if (found3 != null) {
    ...
    for (let i = 0; i < lista.lista_historial.length; i++) {
      ...
      h1 =
        '<li><button class="ver_cambio" onclick="ver_det_historial(' +
        i.toString() +
        ')">Fecha: ' +
        historial.fecha +
        " | Revisor: " +
        ...
        "</button>";
      h2 =
        '<table style="display:None" id = "detalle_' +
        i.toString() +
        '" class="tab_h"><thead><tr><th class="th_h">Descripcion</th><th class="th_h">Antes</th><th class="th_h">Despues</th></tr></thead><tbody>';
      h3 = "";
